...
class OddsApiCallerMixin:
    ...
    def parse_market_info(
        self, market_name: str, market_serial: int, response_json: list[dict]
    ) -> pd.DataFrame:
        entries = []
    ...
    def show_api_requests_used(self) -> None:
        print(
            f"You have used {self.requests_used} calls and have {self.requests_remaining} remaining."
        )
...
class TeamRankingsScraperMixin:
    ...
    def _build_team_tr_table_key(self) -> dict[str, str]:
        ...
        return {
            x: y
            for x, y in zip(self.teams_df["TR_TABLE_NAME"], self.teams_df["SERIAL"])
        }
    ...
    def _build_tr_url_ref_dict(self) -> dict[str, str]:
        ...
        return {
            team_name: url_name
            for team_name, url_name in zip(
                self.teams_df["TEAM_NAME"], self.teams_df["TR_URL_NAME"]
            )
        }
    ...
    def _build_opponent_map(self) -> dict[str, str]:
        ...
        """
        return {
            team_tr_name: url_name
            for team_tr_name, url_name in zip(
                self.teams_df["TR_TABLE_NAME"], self.teams_df["TEAM_NAME"]
            )
        }
    ...
    def _call_tr_ppg_and_oppg(self, league) -> list[pd.DataFrame]:
        ...
        ppg = pd.read_html(
            f"https://www.teamrankings.com/{league.lower()}/stat/points-per-game"
        )
        oppg = pd.read_html(
            f"https://www.teamrankings.com/{league.lower()}/stat/opponent-points-per-game"
        )
...
class Sports_Odds_DB_Packer(OddsApiCallerMixin, TeamRankingsScraperMixin):
    ...
    def __init__(self, db_name):
        ...
        self.db_manager: SqliteDBManager = SqliteDBManager(
            db_name
        )  # define the database you're calling from/to
        ...
        self.combined_market_odds_df: pd.DataFrame = (
            None  # Combine odds_df and games_table info
        )
        self.future_games_checked: bool = (
            False  # check if future games have been accounted for yet
        )

        # League specified in subclasses
        self.team_serial_ref_dict: dict[str, str] = (
            self._build_team_serial_ref()
        )  # League specified in subclasses
    ...
    def _query_last_db_updated_date(self) -> str:
        ...
        """
        db_last_updated_date: str = self.db_manager.fetch_records(
            f"""SELECT MAX(LAST_UPDATED_DATE) FROM DB_UPDATE_RECORDS
                                          WHERE LEAGUE_SERIAL = {self.league_serial}""",
            fetchstyle="one",
        )[0][0]
    ...
    def _query_most_recent_game_date(self) -> str:
        ...
        Returns:
          date (str): The last date the db was updated in %Y-%m-%d %H:%M:%S
        """
        most_recent_game_date = self.db_manager.fetch_records(
            f"""SELECT MAX(GAME_DATE) FROM GAMES
                                               WHERE LEAGUE_SERIAL = {self.league_serial}""",
            fetchstyle="one",
        )[0][0]
    ...
    def _get_sport(self, league: str) -> str:
        ...
        """
        sport: str = self.db_manager.fetch_records(
            f"SELECT ODDS_URL_NAME FROM LEAGUES WHERE NAME ='{league}'",
            fetchstyle="one",
        )
    ...
    def _get_league_serial(self, league: str) -> int:
        ...
        Args:
          League (str): The name of the league you are querying from the DB.
        """
        serial: int = self.db_manager.fetch_records(
    ...
    def _build_team_serial_ref(self) -> dict[str, int]:
        ...
        return {
            team_name: serial
            for team_name, serial in zip(
                self.teams_df["TEAM_NAME"], self.teams_df["SERIAL"]
            )
        }
    ...
    def insert_new_db_updated_date(self) -> None:
        ...
        self.db_manager.insert_table_records(
            "DB_UPDATE_RECORDS", [(now, self.league_serial)]
        )
    ...
    def is_new_week(self) -> bool:
        ...
        try:
            ...
            raise ValueError(
                f"Invalid date format for last_updated_date: {last_updated_date}. Expected format: {date_format}"
            ) from e
    ...
    def get_odds_df(self) -> None:
        ...
        for serial, market_name in markets:
            market_dfs.append(
                self.parse_market_info(market_name, serial, response_json)
            )

        self.odds_df = pd.concat(market_dfs, axis=0)
        self.odds_df["home_serial"] = self.odds_df["Home_Team"].apply(
            self.get_team_serial
        )
        self.odds_df["away_serial"] = self.odds_df["Away_Team"].apply(
            self.get_team_serial
        )
        self.odds_df = self.odds_df[
            [
                "Home_Team",
                "home_serial",
                "Away_Team",
    ...
    def populate_future_game_data(self, debug: bool = False) -> None:
        ...
        if not self.league_serial:
            raise AttributeError(
                "self.league_serial must be populated to use this function."
            )
        if self.odds_df.empty:
            raise AttributeError(
                "You haven't yet called the odds api info to compare this with."
            )
        ...
        try:
            if self.most_recent_game_date:
                unmade_games: pd.DataFrame = odds_df_copy[
                    (odds_df_copy["Date"] > self.most_recent_game_date)
                    & odds_df_copy["MARKET_SERIAL"]
                    == market_id
                ]
            else:
                unmade_games: pd.DataFrame = odds_df_copy[
        ...
        unmade_games = unmade_games[
            ["home_serial", "away_serial", "Date", "league_serial"]
        ]
    ...
    def get_uid(
        self,
        df: pd.DataFrame,
        home_serial_col_name: str,
        away_serial_col_name: str,
        date_col_name: str,
        debug: bool = False,
    ) -> pd.DataFrame:
        """
        ...
        try:
            if not debug:
                df[date_col_name] = pd.to_datetime(
                    df[date_col_name], format="%Y-%m-%d %H:%M:%S"
                )
            else:
                copied_df = copy.deepcopy(df)
                copied_df[date_col_name] = pd.to_datetime(
                    copied_df[date_col_name], format="%Y-%m-%d %H:%M:%S"
                )
                copied_df["day"] = copied_df[date_col_name].dt.date.astype(
                    str
                )  # Extracts the day (YYYY-MM-DD)
                print(f"Day format of col: {copied_df['day'].values[0]}")
                copied_df["time"] = copied_df[date_col_name].dt.time.astype(
                    str
                )  # Extracts the time (HH:MM:SS)
            ...
            raise ValueError(
                f"Date column '{date_col_name}' contains invalid formats. Expected format is '%Y-%m-%d %H:%M:%S'. Error: {e}"
            )

        # Extract day and time
        df["day"] = df[date_col_name].dt.date.astype(
            str
        )  # Extracts the day (YYYY-MM-DD)
        df["time"] = df[date_col_name].dt.time.astype(
            str
        )  # Extracts the time (HH:MM:SS)
    ...
    def _combine_odds_api_and_games_dfs(self) -> pd.DataFrame:
        ...
        if not getattr(self, "odds_called", True):
            try:
                ...
                raise AttributeError(
                    f"Odds api df not yet created. Use self.get_odds_df to fill. Error: {e}"
                )

        else:
            copied_odds_df: pd.DataFrame = copy.deepcopy(self.odds_df)

            copied_odds_df = self.get_uid(
                copied_odds_df, "home_serial", "away_serial", "Date"
            )

            games_df = self.db_manager.dataframe_query("select * from GAMES")
            games_df = self.get_uid(
                games_df, "HOME_TEAM_SERIAL", "AWAY_TEAM_SERIAL", "GAME_DATE"
            )
    ...
    def market_odds_have_changed(self, api_row: pd.Series, existing_row: tuple) -> bool:
        ...
        return (
            api_row["SPREAD_HOME"] != existing_row.SPREAD
            or api_row["TOTAL"] != existing_row.TOTAL
            or api_row["H2H_HOME"] != existing_row.H2H_HOME
            or api_row["H2H_AWAY"] != existing_row.H2H_AWAY
        )

    def get_changed_market_odds(
        self, existing_data: pd.DataFrame, api_data: pd.DataFrame
    ) -> pd.DataFrame:
        new_rows = []
    ...
    def refresh_market_odds_table(self, debug: bool = False) -> None:
        ...
        if getattr(self, "odds_called", False):
            try:
                ...
                raise Exception(
                    f"Could not get odds_df. Troubleshoot starting there. Error was: {e}"
                )
        ...
        if not hasattr(self, "combined_market_odds_df"):
            try:
                ...
                raise Exception(
                    f"Could not get combined_market_odds_df. Troubleshoot starting there. Error was: {e}"
                )
        # Execute assuming that all the correct info is in place to refresh any changed odds data.
        else:
            existing_mkt_odds: pd.DataFrame = (
                self.get_existing_market_odds_records_for_called_games()
            )
            api_mkt_odds: pd.DataFrame = copy.deepcopy(self.combined_market_odds_df)
            api_mkt_odds = api_mkt_odds.drop(
                "Date", axis=1
            )  # drop date column for comparison

            new_entries = self.get_changed_market_odds(
                existing_data=existing_mkt_odds, api_data=api_mkt_odds
            )
            ...
            if new_entries.empty:
                ...
                try:
                    self.db_manager.df_to_db(
                        "GAME_MARKET_ODDS", new_entries, debug=debug
                    )
                except Exception as e:
                    raise RuntimeError(
                        f"Failed to insert records for market odds tabel refresh function:\n {e}"
                    )
    ...
    def pack_odds_table(self, debug: bool = False) -> None:
        ...
        if most_recent_game_date_has_odds:
            most_recent_game_date_has_odds = most_recent_game_date_has_odds[0][
                0
            ]  # unpack val from list of tuples
            # Filter market odds by the game date
            odds_to_add: pd.DataFrame = self.combined_market_odds_df[
                (self.combined_market_odds_df["Date"] > most_recent_game_date_has_odds)
            ]
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to insert records for market odds pack odds table function:\n {e}"
            )
...
class NFL_Data_Packer(Sports_Odds_DB_Packer):
    ...
    def get_predicted_scores(
        self, home_serial: int = None, away_serial: int = None, debug: bool = False
    ) -> list[float, float]:
        """
        ...
        if not hasattr(self, "team_ppg_data"):
            try:
                ...
                raise AttributeError(
                    f"You weren't able to retrieve the necessary self.team_ppg_data attr.\n {e}"
                )
        team_ppg_data = self.team_ppg_data
        # Pre-filter the DataFrame for both teams
        filtered_data = team_ppg_data[
            team_ppg_data["SERIAL"].isin([away_serial, home_serial])
        ]
        ...
        away_stats = [
            away_record.PPG_2024,
            home_record.OPPG_2024,
            away_record.PPG_AWAY,
            home_record.OPPG_HOME,
            away_record.PPG_LAST_3,
            home_record.OPPG_LAST_3,
        ]

        home_stats = [
            home_record.PPG_2024,
            away_record.OPPG_2024,
            home_record.PPG_HOME,
            away_record.OPPG_AWAY,
            home_record.PPG_LAST_3,
            away_record.OPPG_LAST_3,
        ]

        weights = [0.1, 0.1, 0.2, 0.2, 0.2, 0.2]

        # Calculate weighted averages
        away_team_score_prediction = round(
            self.get_weighted_average(away_stats, weights), 2
        )
        home_team_score_prediction = round(
            self.get_weighted_average(home_stats, weights), 2
        )
    ...
    def get_predictions_for_games(self) -> list[tuple]:
        ...
        for row in games_get_odds.itertuples():
            predicted_home_points, predicted_away_points = self.get_predicted_scores(
                home_serial=row.HOME_TEAM_SERIAL, away_serial=row.AWAY_TEAM_SERIAL
            )

            game_own_odds_rows.append(
                GameOwnOddsRow(row.SERIAL, predicted_home_points, predicted_away_points)
            )
    ...
    def pack_own_odds_predictions(self, debug: bool = False) -> None:
        ...
        current_own_odds: pd.DataFrame = self.db_manager.dataframe_query(
            "select * from GAME_OWN_ODDS"
        )
        ...
        self.db_manager.insert_table_records(
            "GAME_OWN_ODDS", game_own_odds_tuples, debug=debug
        )
    ...
    def format_team_game_history(self, df: pd.DataFrame):
        ...
        df[["Result", "Points", "Opp Points"]] = df["Result"].str.extract(
            r"(\w+)\s+(\d+)-(\d+)"
        )
        ...
        df["Date"] = pd.to_datetime(df["Date"] + f"/{current_year} ").dt.strftime(
            self.db_manager.db_date_format
        )
    ...
    def get_all_games_data(self) -> None:
        ...
        games_scrapers: list[Team_Games_Scraper] = (
            self.build_scrapers()
        )  # uses teams_df data
        ...
        for scraper in games_scrapers:
            # retrieve raw data
            nfl_team_data = pd.read_html(
                f"https://www.teamrankings.com/nfl/team/{scraper.tr_url_name}/"
            )
            ...
            if most_recent_game_w_outcome:
                games_data = games_data[
                    games_data["Date"] >= most_recent_game_w_outcome
                ]

            # Check if DataFrame is empty
            if games_data.empty:
                print(
                    f"{scraper.team_name} has no game updates since {most_recent_game_w_outcome}"
                )
                time.sleep(10)
            ...
            games_data["away_serial"] = games_data["Opponent"].apply(
                self.get_team_serial
            )
            ...
            print(
                f"{scraper.team_name} added {len(games_data)} games since {most_recent_game_w_outcome}."
            )
            # Respect crawl delay
            time.sleep(10)
    ...
    def pack_games_data(self, acknowledged: bool = False) -> None:
        ...
        if not self.games_data_retrieved:
            try:
                ...
                raise ValueError(
                    "Game data not yet packed, run self.pack_games_data()."
                )
        if not acknowledged:
        ...
        if self.most_recent_game_date:
            games_table_data = games_table_data[
                games_table_data["Date"] >= self.most_recent_game_date
            ]
    ...
    def pack_game_outcomes_data(self, debug: bool = False) -> None:
        ...
        for team, df in game_outcomes_table_data.items():
            ...
            df = df[
                ["Date", "home_serial", "away_serial", "Result", "Points", "Opp Points"]
            ]
            ...
            joined_df["winner_serial"] = joined_df.apply(
                lambda row: row["home_serial"]
                if row["Result"] == "W"
                else row["away_serial"],
                axis=1,
            )
    ...
    def full_pack(
        self, pack_all_games_data: bool = False, acknowledged: bool = False
    ) -> None:
